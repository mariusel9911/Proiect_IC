                ...
                <div className="flex flex-col gap-4">
                  {filteredProviders.length > 0 ? (
                    filteredProviders.map((provider) => {
                      // Find the provider's specific options for this service
                      const serviceOffering = provider.serviceOfferings?.find(
                        (offering) => offering.service.toString() === serviceId
                      );

                      // For backward compatibility
                      let providerOptions = currentService.options;

                      // If we have specific options for this service, use them
                      if (serviceOffering && serviceOffering.options) {
                        providerOptions = serviceOffering.options.map(
                          (option) => ({
                            _id: option.optionId,
                            id: option.optionId,
                            name: option.name,
                      ...
                      return (
                        <Link
                          ...
                          state={{
                            provider: {
                              ...provider,
                              // Include service and options data
                              service: currentService,
                              options: providerOptions,
                              // Use service-specific description if available
                              description:
